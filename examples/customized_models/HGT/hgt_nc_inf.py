...
from graphstorm.utils import setup_device, get_rank
...
class HGT(gsmodel.GSgnnNodeModelBase):
    ...
    def restore_model(self, restore_model_path):
        if restore_model_path is not None:
            self.adapt_ws = self.adapt_ws.module if isinstance(self.adapt_ws, DistributedDataParallel) else self.adapt_ws
            self.gcs = self.gcs if isinstance(self.gcs, DistributedDataParallel) else self.gcs
            self.out = self.gcs if isinstance(self.out, DistributedDataParallel) else self.out

        if torch.__version__ < "1.13.0":
            checkpoint = torch.load(os.path.join(model_path, 'model.bin'), map_location='cpu')
        else:
            checkpoint = torch.load(os.path.join(model_path, 'model.bin'), map_location='cpu', weights_only=True)
            
        if 'adapt_ws' in checkpoint and self.adapt_ws is not None:
            self.adapt_ws.load_state_dict(checkpoint['adapt_ws'])
        if 'gcs' in checkpoint and self.gcs is not None:
            self.gcs.load_state_dict(checkpoint['gcs'], strict=False)
        if 'out' in checkpoint and self.out is not None:
            self.out.load_state_dict(checkpoint['out'])

    def save_model(self, model_path):
        if get_rank() == 0:
            # save_model(model_path, self.adapt_ws, self.gcs, self.out)
            model_states = {}
            if self.adapt_ws is not None and isinstance(self.adapt_ws, nn.Module):
                model_states['adapt_ws'] = self.adapt_ws.state_dict()
            if self.gcs is not None and isinstance(self.gcs, nn.Module):
                model_states['gcs'] = self.gcs.state_dict()
            if self.out is not None and isinstance(self.out, nn.Module):
                model_states['out'] = self.out.state_dict()
            
            os.makedirs(model_path, exist_ok=True)
            torch.save(model_states, os.path.join(model_path, 'model.bin'))
...
if __name__ == '__main__':
    ...
    argparser.add_argument("--yaml-config-file",
                           "--cf", type=str, required=True,
